...
- **Six MediatR behaviors, followed by the Handler**: Validation → Logging → AuditLog → Authorization → Transaction → OutboxFlush → Handler. `Validation`, `Logging` and `Transaction` are live; `AuditLog`, `Authorization` and `OutboxFlush` ship as shells whose registration order is the binding part.
...
- **Contract:** `EntitlementProjection_Shape_IsStable` snapshots the serialised projection against a checked-in `entitlement-v1.schema.json`. The same file is checked into LearnStack and asserted there independently, per [ADR-0034](https://github.com/HodeTech/LearnStack/blob/main/docs/decisions/0034-hub-contract-surface-invariant.md).
