...
Hub docs cite these via sibling-relative paths. From a file one level deep in `docs/`, that is `../../LearnStack/...` — with that capitalisation, because the directory on disk is `LearnStack`.

- [ADR-0019 LearnStack Hub](../../LearnStack/docs/decisions/0019-learnstack-hub.md) — why the Hub is a separate repository
- [ADR-0034 Hub Contract Surface Invariant](../../LearnStack/docs/decisions/0034-hub-contract-surface-invariant.md) — the two invariants that replaced the "closed at four endpoints" rule, and the authoritative endpoint table
- [ADR-0035 Demand-Gated Infrastructure](../../LearnStack/docs/decisions/0035-demand-gated-infrastructure.md) — why the Hub integration waits on a written trigger
- [ADR-0033 Audit Durability Model](../../LearnStack/docs/decisions/0033-audit-durability-model.md) — MUST-class audit as durable intent (supersedes ADR-0016)
...
- [Architecture 24 LearnStack Hub](../../LearnStack/docs/architecture/24-learnstack-hub.md) — the deep dive
...
Hub follows LearnStack's [Standards corpus](../../LearnStack/docs/standards/) by reference. Hub does not maintain its own standards. If a Hub-internal-only rule emerges (for example a Stripe webhook idempotency strategy), it lands as a Hub-internal ADR (`HUB-NNNN`), not as a standards file.
